...
void CallbackHandler(const sensor_msgs::PointCloud2ConstPtr &msgs){
    ...
    // double timestamp=msgs->header.stamp.toSec();
    // ROS_INFO("%20lf",timestamp);
    ...
    for(int i=0;i<pointcloud_size;i++){
        ...
        if(last_frame_idx != selected_img_frame_idx){
            ...
            // log_file2<<fixed<<timestamp<<"   "<<pointcloud_size<<"  "<< img_list_vec[selected_img_frame_idx] <<endl;
        }
...
int main(int argc, char** argv){
    ...
    // log_file<<"image path: "<<img_path<<endl;
